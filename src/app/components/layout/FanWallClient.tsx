...
export default function FanWallClient({
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        !(event.target as Element).closest(".fanwall-menu") &&
        !(event.target as Element).closest(".fanwall-menu-button")
      ) {
        setOpenMenuId(null);
      }
    };
  ...
  async function submitMessage(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/fanwall/messages", {
        ...
        body: JSON.stringify({
          body: body.trim(),
          title: isAdmin ? title.trim() : undefined,
          nickname: sessionUser ? undefined : nickname.trim(),
          contact: sessionUser ? undefined : contact.trim(),
        }),
      });
      ...
      {pinnedMessage && (
        <div className="bg-neutral-200 py-4 dark:bg-[#444444]">
          <div className="group relative flex flex-row items-center justify-baseline gap-2 px-2 md:gap-4 md:px-6">
            <div>
              <Image
                src={pinnedMessage.user?.image || DEFAULT_AVATAR}
                alt="profile-avatar"
            ...
            <div className="flex flex-col space-y-1">
              {pinnedMessage.title && (
                <h5 className="font-medium">{pinnedMessage.title}</h5>
              )}
              <p>
      ...
      <div className="bg-neutral-200 py-4 dark:bg-neutral-700">
        ...
        {otherMessages.map((message) => {
          const authorName =
            ...
            message.nickname ||
            "Guest";
          const avatarSrc = message.user?.image || DEFAULT_AVATAR;
          ...
          return (
            <div
              key={message.id}
              className="group relative flex flex-row items-start justify-baseline gap-2 px-2 py-4 md:gap-4 md:px-6"
            >
      ...
      >
        ...
        <div className="mt-3 flex items-center justify-between gap-3">
          <div className="text-xs text-neutral-500 dark:text-neutral-300">
            {sessionUser ? "You are posting as" + " " + sessionDisplayName : ""}
          </div>
          <button
            type="submit"
